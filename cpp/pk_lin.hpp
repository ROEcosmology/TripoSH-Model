...
gsl_interp_accel * acc_pk_no_wiggle = NULL;
gsl_spline       * spline_pk_no_wiggle = NULL;
...
double f_pk_no_wiggle(double kmag) {
	...
	if( (kmag < pk_kmin) || (kmag > pk_kmax) ) { 
		return 0.0;
	} else if (spline_pk_no_wiggle == NULL || acc_pk_no_wiggle == NULL) {
		return 0.0;
	} else {
		return norm_no_wiggle * gsl_spline_eval(spline_pk_no_wiggle, kmag, acc_pk_no_wiggle);	  
	}
}
...
//	/* set kmin and kmax */
	...
	/* spline interpolation */
	acc_pk_no_wiggle = gsl_interp_accel_alloc();
	spline_pk_no_wiggle = gsl_spline_alloc(gsl_interp_cspline, pk_num_in);
	gsl_spline_init(spline_pk_no_wiggle, kmag_in, pk_in, pk_num_in);
...
double f_pk_no_wiggle_integrand(double kmag) {

	/*****************************************************************/
	//     compute un-normalized linear P(k) 
	//     based on eq.(29) of Eisenstein & Hu (1998)
	//     (no-wiggle approximation)  
	//     fitting formula for no-wiggle P(k) (Eq.[29] of EH98)
	/*****************************************************************/

	/* BOSS fiducial cosology */
	double h = 0.676;
	double omegab = 0.0481425;
	double omega0 = 0.31;
	double Tcmb = 2.7255;
	double n_s = 0.96;

	//double ss = 44.5 * h * log( 9.83 / (omega0*h*h) ) / sqrt( 1.0 + 10.0 * pow(omegab*h*h,0.75) );
    double ss = 44.5 * log( 9.83 / (omega0*h*h) ) / sqrt( 1.0 + 10.0 * pow(omegab*h*h,0.75) );
...
void calcNormalizationNoWiggle(double sigma8, double h, double omegab, double omega0, double Tcmb, double n_s) {
	...
	for(int n = 0; n <= N; n++) {
		...
		double power =  kmag3 * w2 * f_pk_no_wiggle_integrand(kmag * h, h, omegab, omega0, Tcmb, n_s);
	...
	norm_no_wiggle = 1.0; /*sigma8 * sigma8 / integ;*/
...
void finalizeInputPowerSpectrum() {
	...
	gsl_interp_accel_free(acc_pk_no_wiggle);
	acc_pk_no_wiggle = NULL;
	gsl_spline_free(spline_pk_no_wiggle);
	spline_pk_no_wiggle = NULL;
